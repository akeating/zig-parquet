    ...
    return struct {
        ...
        pub fn init(allocator: std.mem.Allocator, length: usize, has_nulls: bool) !Self {
            const validity = if (has_nulls) blk: {
                const bitmap = try allocator.alloc(u8, (length + 7) / 8);
        ...
        /// Set the value at index i to valid (clear null bit)
        ...
        /// Get the value at index i, returning null if the value is null
        pub fn get(self: Self, i: usize) ?T {
            if (self.isNull(i)) return null;
            return self.values[i];
        }

        /// Set the value at index i. If value is null, marks the position as null.
...
/// Set a bit in a validity bitmap
...
/// Clear a bit in a validity bitmap (mark as null)
...
pub fn countNullBits(bitmap: []const u8, len: usize) usize {
    return len - countValidBits(bitmap, len);
}
