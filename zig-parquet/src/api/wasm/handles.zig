...
pub const TypeInfo = struct {
    ...
    pub fn fromZpType(t: c_int) ?TypeInfo {
        return switch (t) {
            c_err.ZP_TYPE_BOOL => .{ .physical = .bool_, .logical = null, .type_length = null },
            c_err.ZP_TYPE_INT32 => .{ .physical = .int32, .logical = null, .type_length = null },
            c_err.ZP_TYPE_INT64 => .{ .physical = .int64, .logical = null, .type_length = null },
            c_err.ZP_TYPE_FLOAT => .{ .physical = .float_, .logical = null, .type_length = null },
            c_err.ZP_TYPE_DOUBLE => .{ .physical = .double_, .logical = null, .type_length = null },
            c_err.ZP_TYPE_BYTES => .{ .physical = .bytes, .logical = null, .type_length = null },

            c_err.ZP_TYPE_STRING => .{ .physical = .bytes, .logical = .string, .type_length = null },
            c_err.ZP_TYPE_DATE => .{ .physical = .int32, .logical = .date, .type_length = null },
            c_err.ZP_TYPE_TIMESTAMP_MILLIS => .{ .physical = .int64, .logical = .{ .timestamp = .{ .is_adjusted_to_utc = true, .unit = .millis } }, .type_length = null },
            c_err.ZP_TYPE_TIMESTAMP_MICROS => .{ .physical = .int64, .logical = .{ .timestamp = .{ .is_adjusted_to_utc = true, .unit = .micros } }, .type_length = null },
            c_err.ZP_TYPE_TIMESTAMP_NANOS => .{ .physical = .int64, .logical = .{ .timestamp = .{ .is_adjusted_to_utc = true, .unit = .nanos } }, .type_length = null },
            c_err.ZP_TYPE_TIME_MILLIS => .{ .physical = .int32, .logical = .{ .time = .{ .is_adjusted_to_utc = false, .unit = .millis } }, .type_length = null },
            c_err.ZP_TYPE_TIME_MICROS => .{ .physical = .int64, .logical = .{ .time = .{ .is_adjusted_to_utc = false, .unit = .micros } }, .type_length = null },
            c_err.ZP_TYPE_TIME_NANOS => .{ .physical = .int64, .logical = .{ .time = .{ .is_adjusted_to_utc = false, .unit = .nanos } }, .type_length = null },
            c_err.ZP_TYPE_INT8 => .{ .physical = .int32, .logical = .{ .int = .{ .bit_width = 8, .is_signed = true } }, .type_length = null },
            c_err.ZP_TYPE_INT16 => .{ .physical = .int32, .logical = .{ .int = .{ .bit_width = 16, .is_signed = true } }, .type_length = null },
            c_err.ZP_TYPE_UINT8 => .{ .physical = .int32, .logical = .{ .int = .{ .bit_width = 8, .is_signed = false } }, .type_length = null },
            c_err.ZP_TYPE_UINT16 => .{ .physical = .int32, .logical = .{ .int = .{ .bit_width = 16, .is_signed = false } }, .type_length = null },
            c_err.ZP_TYPE_UINT32 => .{ .physical = .int32, .logical = .{ .int = .{ .bit_width = 32, .is_signed = false } }, .type_length = null },
            c_err.ZP_TYPE_UINT64 => .{ .physical = .int64, .logical = .{ .int = .{ .bit_width = 64, .is_signed = false } }, .type_length = null },
            c_err.ZP_TYPE_UUID => .{ .physical = .fixed_bytes, .logical = .uuid, .type_length = 16 },
            c_err.ZP_TYPE_JSON => .{ .physical = .bytes, .logical = .json, .type_length = null },
            c_err.ZP_TYPE_ENUM => .{ .physical = .bytes, .logical = .enum_, .type_length = null },
            c_err.ZP_TYPE_FLOAT16 => .{ .physical = .fixed_bytes, .logical = .float16, .type_length = 2 },
            c_err.ZP_TYPE_BSON => .{ .physical = .bytes, .logical = .bson, .type_length = null },
            c_err.ZP_TYPE_INTERVAL => .{ .physical = .fixed_bytes, .logical = null, .type_length = 12 },
            c_err.ZP_TYPE_GEOMETRY => .{ .physical = .bytes, .logical = .{ .geometry = .{} }, .type_length = null },
            c_err.ZP_TYPE_GEOGRAPHY => .{ .physical = .bytes, .logical = .{ .geography = .{} }, .type_length = null },
            else => null,
        };
    }
...
pub const RowWriterHandle = struct {
    ...
    pub fn openCallbackWriter(cb_writer: *CallbackWriter) !*RowWriterHandle {
        const allocator = backing_allocator;
        const handle = try allocator.create(RowWriterHandle);
        handle.* = .{ .allocator = allocator, .backend = .{ .callback = cb_writer } };
        return handle;
    }
